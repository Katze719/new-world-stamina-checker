...
EDITOR_MAILS = ["bot-300@black-beach-453214-f6.iam.gserviceaccount.com", "diekrankenpfleger@gmail.com", "cyradiss1986@gmail.com", "ladylilian96@gmail.com", "pauldorn1234@gmail.com"]
...
async def process_raidhelper_for_payoutlist(sheet, discord_member_list, discord_member_list_orig_names, raidhelper_usernames, event_time, event_type):
    ...
    for i, member in enumerate(discord_member_list):
        ...
        if discord_username_origin not in A_col:
            ...
            # stats
            await sheet.update_cell(row, 1, f'=WENNFEHLER((ZÄHLENWENNS($J$4:$AAA$4; "*Push*";$J$3:$AAA$3;"*Teilnehmer*";J{row}:AAA{row}; ">0") + ZÄHLENWENNS($J$4:$AAA$4; "*Push*";$J$3:$AAA$3;"*Teilnehmer*";J{row}:AAA{row}; "x"))/$A$5; 0)')
            await sheet.update_cell(row, 2, f'=WENNFEHLER((ZÄHLENWENNS($J$4:$AAA$4; "*Krieg*";$J$3:$AAA$3;"*Teilnehmer*";J{row}:AAA{row}; ">0") + ZÄHLENWENNS($J$4:$AAA$4; "*Krieg*";$J$3:$AAA$3;"*Teilnehmer*";J{row}:AAA{row}; "x"))/$B$5; 0)')
            await sheet.update_cell(row, 3, f'=WENNFEHLER((ZÄHLENWENNS($J$3:$AAA$3;"*Raidhelper*";J{row}:AAA{row}; ">0") + ZÄHLENWENNS($J$3:$AAA$3;"*Raidhelper*";J{row}:AAA{row}; "x"))/$C$5; 0)')
            await sheet.update_cell(row, 4, f'=WENNFEHLER((ZÄHLENWENNS($J$3:$AAA$3;"*VOD*";J{row}:AAA{row}; ">0") + ZÄHLENWENNS($J$3:$AAA$3;"*VOD*";J{row}:AAA{row}; "x"))/$D$5; 0)')
